...
import { Mic, MicOff, Video as VideoIcon, VideoOff, PhoneOff, GripVertical, User, GraduationCap, AlertTriangle, Loader2, KeyRound, Clock, Link as LinkIcon, Hash, Image as ImageIcon, Users, Copy, Check } from 'lucide-react';

// 👇 Importando tudo direto da sua lib oficial 👇
...
function SalaDeVideo({ payload, onSair, salaId, setSalaId }) {
  ...
  return (
    <div className="relative w-full h-[100dvh] bg-slate-950 overflow-hidden text-white">
      ...
      {/* Header Responsivo com Botão de Copiar */}
      ...
      <Draggable nodeRef={janelaVideoRef} bounds="parent" defaultPosition={{ x: 0, y: 0 }}>
        <div ref={janelaVideoRef} className={`absolute top-20 right-4 md:top-6 md:right-6 z-20 w-32 sm:w-48 md:w-72 aspect-video bg-slate-900 rounded-lg md:rounded-2xl overflow-hidden shadow-2xl border-2 transition-all duration-150 group cursor-grab active:cursor-grabbing ${isSpeaking ? 'border-emerald-400 shadow-emerald-400/30 shadow-lg' : 'border-white/5'}`}>
          <div className="absolute inset-y-0 left-0 w-4 md:w-6 flex items-center justify-center bg-slate-800/80 opacity-0 group-hover:opacity-100 transition-opacity"><GripVertical className="text-white/40 w-3 h-3 md:w-4 md:h-4" /></div>
          {isCamOn ? <LocalVideo className="w-full h-full object-cover scale-x-[-1]" /> : <div className="w-full h-full flex items-center justify-center bg-slate-900"><Avatar nome={payload.nome} avatar={payload.avatar} size="sm" /></div>}
          <div className="absolute bottom-1 right-1 md:bottom-2 md:right-2 px-1.5 md:px-2 py-0.5 bg-black/50 backdrop-blur-sm rounded text-[8px] md:text-xs text-white/70 flex items-center gap-1">
            {!isMicOn && <MicOff className="text-red-400 w-2 h-2 md:w-2.5 md:h-2.5" />}{!isCamOn && <VideoOff className="text-red-400 w-2 h-2 md:w-2.5 md:h-2.5" />}
      ...
      <div className="absolute bottom-6 md:bottom-10 left-1/2 -translate-x-1/2 z-30 flex items-center gap-3 md:gap-4 bg-slate-900/90 backdrop-blur-lg px-4 py-3 md:px-8 md:py-5 rounded-full shadow-2xl border border-white/10 w-auto whitespace-nowrap">
        <ControlBtn onClick={toggleMic} active={isMicOn}>{isMicOn ? <Mic className="w-5 h-5 md:w-6 md:h-6" /> : <MicOff className="w-5 h-5 md:w-6 md:h-6" />}</ControlBtn>
        <ControlBtn onClick={toggleCam} active={isCamOn}>{isCamOn ? <VideoIcon className="w-5 h-5 md:w-6 md:h-6" /> : <VideoOff className="w-5 h-5 md:w-6 md:h-6" />}</ControlBtn>
        <div className="w-px h-6 md:h-10 bg-white/10 mx-1 md:mx-2" />
        <button onClick={sair} className="flex items-center gap-2 md:gap-3 px-5 py-2.5 md:px-8 md:py-4 rounded-full bg-red-500 hover:bg-red-600 text-white font-semibold text-sm md:text-lg shadow-lg transition-all hover:scale-105"><PhoneOff className="w-4 h-4 md:w-6 md:h-6" /> <span className="hidden sm:inline">Sair</span></button>
      </div>
    </div>
  );
}
