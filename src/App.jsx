...
function SalaDeVideo({ payload, onSair, salaId, setSalaId }) {
  ...
  const toggleRemoteMute = () => setIsRemoteMuted(!isRemoteMuted);
  ...
  return (
    <div className="relative w-full h-[100dvh] bg-slate-950 overflow-hidden text-white">
      ...
      <div className={`absolute inset-0 z-0 transition-all duration-150 ${remoteSpeaking ? 'ring-4 ring-inset ring-emerald-400/40' : ''}`}>
        <RemoteVideo className="w-full h-full object-cover" fallbackText="" muted={isRemoteMuted} />
      </div>

      {/* JANELA DE PREVIEW (VÍDEO LOCAL) - AGORA COM OS CONTROLES DELA */}
      <Draggable nodeRef={janelaVideoRef} bounds="parent" defaultPosition={{ x: 0, y: 0 }} handle=".drag-handle">
        <div 
          ...
          className={`
            ...
            bg-slate-900 rounded-2xl md:rounded-2xl overflow-hidden shadow-2xl border-2 transition-all duration-150 group 
            ${isSpeaking ? 'border-emerald-400 shadow-emerald-400/30 shadow-lg' : 'border-white/5'}
          `}
        >
          {/* ÁREA DE ARRASTAR (DRAG HANDLE) - Fica no topo da janelinha */}
          ...
          {/* CONTROLES LOCAIS (MEU MIC / MINHA CAM) DENTRO DA JANELINHA */}
      ...
      {/* BOTTOM BAR (CONTROLES GERAIS E REMOTOS) */}
      <div className="absolute bottom-6 md:bottom-10 left-1/2 -translate-x-1/2 z-30 flex items-center gap-4 md:gap-6 bg-slate-900/90 backdrop-blur-lg px-6 py-3 md:px-8 md:py-4 rounded-full shadow-2xl border border-white/10 w-auto whitespace-nowrap">
        
        {/* Controle Remoto (Mutar a outra pessoa) */}
        <div className="flex flex-col items-center gap-1.5">
          <ControlBtn onClick={toggleRemoteMute} active={!isRemoteMuted}>
        ...
        <div className="flex flex-col items-center gap-1.5">
          <button onClick={sair} className="w-10 h-10 md:w-12 md:h-12 flex items-center justify-center rounded-xl bg-red-500 hover:bg-red-600 text-white shadow-lg shadow-red-500/20 transition-all active:scale-95" title="Encerrar Chamada">
            <PhoneOff className="w-5 h-5 md:w-6 md:h-6" />
          </button>
          <span className="text-[8px] md:text-[9px] text-red-400 font-bold tracking-wider uppercase">
            Encerrar
          </span>
        </div>
